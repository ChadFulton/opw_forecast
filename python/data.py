...
class OPWData(pd.DataFrame):
    def __init__(self, *args, **kwargs):
        ...
        # Setup the index
        self.index = pd.date_range(start='1960-01-01', periods=self.shape[0],
                                   freq='MS')
        ...
        self._state_growth_columns = [
            (
                '%s_emp_growth' % re.sub(pattern, '', state)
            ).lower().replace(' ', '_')
            for state in self._state_emp_columns
        ]
        self._state_growth_columns[27] = '_'.join(
            self._state_growth_columns[27].split('_')[1:]
        )
        self._state_growth_columns[49] = self._state_growth_columns[49].replace('.', '')
        self._state_names = [
            ' '.join(col.split('_')[:-2]).title()
            for col in self._state_growth_columns
        ]
        ...
        self['agg_ip_growth'] = self.growth_rate(
            self['U.S. Industrial Production']
        )

        # Setup the state employment growth columns
        self[self._state_growth_columns] = self.growth_rate(
            self[self._state_emp_columns]
        )
        ...
        self._financial_names = [
            'Federal Funds Rate', 'S&P 500 Return', 'Term Spread'
        ]

        self._growth_columns = [
            'agg_emp_growth', 'agg_ip_growth'
        ] + self._state_growth_columns

        self._growth_names = [
            'Agg. Emp. Growth', 'Agg. IP Growth'
        ] + self._state_names

        self._exog_columns = (
            ['intercept'] + self._financial_columns + self._growth_columns
        )

        self._exog_names = (
            ['Constant'] + self._financial_names + self._growth_names
        )
